...
本复现支持**多种视觉-语言模型**及**随机基线**，统一为零样本 MCQ 接口，可同时跑多个模型并生成对比报告。

**本次实验采用的 4 个模型**（与 `results/MM-UAVBench_report.txt` 一致）：

| 模型 id | 模型名称 | 权重来源 | 说明 |
|---------|----------|----------|------|
| **random_baseline** | Random baseline | — | 在选项字母上均匀随机选择，固定种子可复现，作为下界对比。 |
| **clip_vitb32** | CLIP ViT-B/32 | `openai/clip-vit-base-patch32` | 轻量、速度快，图像-文本相似度 → argmax 选项。 |
| **clip_vitl14** | CLIP ViT-L/14 | `openai/clip-vit-large-patch14` | 更大 backbone，同上相似度推理。 |
| **qwen2vl_2b** | Qwen2-VL-2B-Instruct | `Qwen/Qwen2-VL-2B-Instruct` | 生成式 VLM，根据图像+题目生成选项字母，适合复杂推理。 |

脚本中另支持（可选）：siglip_base、qwen2vl_7b、qwen3vl_8b 等，见 `run_mmuavbench_official_tasks.py` 内 `AVAILABLE_MODELS`。

**共同约定**：

- **输入**：单张 RGB 图像（经各模型对应的预处理）；选项为英文原文。
- **输出**：CLIP/SigLIP 为图像-选项相似度 argmax；Qwen2-VL 为生成字母解析；random_baseline 为随机选项字母。
- **调用方式**：命令行 `--models random_baseline clip_vitb32 clip_vitl14 qwen2vl_2b` 可指定多个模型，脚本会依次加载、对同一批任务逐题推理，并在报告中输出**任务×模型**汇总表。
...
无 GPU 时建议仅跑轻量模型（如 `--models clip_vitb32`）或 random_baseline，或对 Qwen2-VL 使用较小 `--max-samples` 与 `--cpu-threads`。
...
**加速实验**：使用 `--fast` 可启用快速预设（每任务 10 题、clip_vitb32 + siglip_base，并自动设置 CPU 线程），适合快速验证流程。详见 README「怎么跑」。
